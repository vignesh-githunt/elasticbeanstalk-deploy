...
const ForumDiscussion = (props) => {
    const [replyCollapse, setReplyCollapse] = useState(false);
    
    const params = props.match.params;

    const toggleCollapse = e => setReplyCollapse(!replyCollapse)
    return (
        <ContentWrapper>
                <div className="content-heading">
                    <div>Forum - Discussion
                        <small>Topic ID: {params.topid}</small>
                    </div>
                    <div className="ml-auto">
                        <Link to={`/forum/topics/${params.catid}`} className="btn btn-sm btn-secondary text-sm">&lt; back</Link>
                    </div>
                </div>
                ...
                <div className="text-center mb-3">
                    <Button onClick={toggleCollapse} color="primary">Reply to this topic</Button>
                </div>
                <Collapse isOpen={replyCollapse}>
