...
const BlogArticleView = () => {
    const INITIAL_STATE ={ categories : [],tags : [], reviewers : [],editorState : EditorState.createWithContent(initialEditorContent)};
    const [myState, setMyState] = useState(INITIAL_STATE);
    const {categories,tags,reviewers,editorState} = myState 
   
    const handleChangeSelect = (name, newVal) => {
        setMyState({[name]: newVal})
    }

    const onEditorStateChange = editorState => {
       setMyState({editorState})
    }
                ...
                <Col lg={ 3 }>
                    <Card body className="card-default">
                        ...
                        <Select
                            ...
                            simpleValue
                            value={reviewers}
                            onChange={handleChangeSelect('reviewers')}
                            options={REVIEWERS}
                        />
