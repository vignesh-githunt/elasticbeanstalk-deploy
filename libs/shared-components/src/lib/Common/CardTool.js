...
export const CardTool = (props) => {
  ...
  const handleDismiss = (e) => {
    ...
    const card = getCardParent(toolElement.current);
    ...
    props.onRemove(card, confirmRemove);
  };
  ...
  const handleRefresh = (e) => {
    ...
    showSpinner(card, props.spinner.split(" "));
    // event to remove spinner when refres is done
    props.onRefresh(card, done);
  };

  return (
    <div ref={toolElement} className="card-tool float-right">
      {props.refresh && (
