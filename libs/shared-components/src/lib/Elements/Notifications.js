...
import { Row,
    ...
    const PopoverItem = (props) => {
        ...
        return (
            <span>
                <Button className="mr-1" color="secondary" id={'Popover-' + props.id} onClick={toggle}>{props.item.text}</Button>
                <Popover placement={props.item.placement} isOpen={popoverOpen} target={'Popover-' + props.id} toggle={toggle}>
    ...
    const TooltipItem = (props) => {
        const _id = useState('id4tooltip_'+new Date().getUTCMilliseconds()+(Math.floor(Math.random()*10) + 1));
        const [tooltipOpen,setTooltipOpen] = useState(false);
        const toggle = e => setTooltipOpen(!tooltipOpen)
        return [
            <Tooltip {...props} isOpen={tooltipOpen} toggle={toggle} target={_id} key='1'>
                {props.content}
            </Tooltip>,
    ...
    const Notifications = () => {
        ...
        const [toasterPos,setToasterPos] = useState('top-right');
        const [toasterType,setToasterType] = useState('info');
        ...
        const onToastPosChanged = e => {
            setToasterPos(e.currentTarget.value)
        }

        const onToastTypeChanged = e => {
            setToasterType(e.currentTarget.value)
        }
                ...
                <Card className="card-default">
                    ...
                    <CardBody>
                        <Row>
                                ...
                                <Row>
                                    <Col md="6">
                                        ...
                                        {TOAST_POSITIONS.map((pos, i) => (
                                            <FormGroup check key={i}>
                                                <Label check>
                                                  <Input type="radio" name="tPosition" value={pos} checked={toasterPos === pos} onChange={onToastPosChanged}/>{' '}
                                                  {pos}
                                                </Label>
                                            </FormGroup>
                                        ))}
                                    ...
                                    <Col md="6">
                                        ...
                                        {TOAST_TYPES.map((type, i) => (
                                            <FormGroup check key={i}>
                                                <Label check>
                                                  <Input type="radio" name="tType" value={type} checked={toasterType === type} onChange={onToastTypeChanged}/>{' '}
                                                  {type}
                                                </Label>
                                            </FormGroup>
                                        ))}
                            ...
                            <Col md="6">
                                <Button color="primary" className="mt-3 mt-sm-0" onClick={notify}>Notify !</Button>
                                <ToastContainer />
                            </Col>
                        </Row>
                    </CardBody>
                   ...
                   <div className="col-xl-6">
                      ...
                      <Card className="card-default">
                         ...
                         <CardBody>
                            ...
                            <Button color="primary" onClick={toggleModal}>Open Modal Example</Button>
