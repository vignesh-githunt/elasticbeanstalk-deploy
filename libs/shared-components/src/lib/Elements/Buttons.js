...
const Buttons = (props) => { 
   ...
   const handleSelect=(event, selectedEvent) => {
       setActivePage({ [activePage]: selectedEvent.eventKey})
  }
  const toggleDD = dd => {
    setActivePage({[dd]: !props[dd]})
  }

  const renderDropdownButton = (title, i) => {
    return (
        <ButtonDropdown isOpen={props[`ddSingle${i}`]} toggle={() => toggleDD(`ddSingle${i}`)} key={ i } id={ `dropdown-basic-${i}` }>
 ...
 const renderDropdownSplitButton = (title, i) => {
    return (
        <ButtonDropdown isOpen={props[`ddSplit${i}`]} toggle={() => toggleDD(`ddSplit${i}`)}  key={ i } id={ `split-button-basic-${i}` }>
        ...
        <Row>
            ...
            <Col md={ 6 }>
                ...
                <CardWithHeader header="Split button dropdown">
                    <ButtonToolbar>
                        { BUTTONS.map(renderDropdownSplitButton) }
                    </ButtonToolbar>
                </CardWithHeader>
                { /* END Card */ }
            </Col>
