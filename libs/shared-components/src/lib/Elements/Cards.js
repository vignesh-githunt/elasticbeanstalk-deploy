...
const Cards = ()=>{
    const activeTab = '1';
    const key = 1;
    const accordionState = [true, false, false];
    const oneAtATime = true;
    const [myState, setMyState] = useState(activeTab,key,accordionState,oneAtATime);
    const {activeTab,key,accordionState,oneAtATime} = myState

    const toggleTab = tab => {
        if (activeTab !== tab) {
            setMyState({[activeTab]: tab})
        }
    }
    ...
    const onCheckChange = e => {
        setMyState({[oneAtATime]: e.currentTarget.checked})
    }
        ...
        <ContentWrapper>
            ...
            <div>
                <div className="custom-control custom-checkbox mb-3">
                  <input type="checkbox" id="customCheck1" className="custom-control-input" checked={oneAtATime} onChange={onCheckChange}/>
